...
set.seed(321)
start_time <- Sys.time() 
mymodel.1.full <- rfSLAM::rfsrc(i.sev_died.1 ~., data = train, nodesize = node.size, ntree = 100, na.action = "na.impute", splitrule="poisson.split1", risk.time = train_b$rt_1p, stratifier=train_b$q6, membership = TRUE, bootstrap = "by.user", samp = rfslam.samp.1)
end_time <- Sys.time()

## obtain predicted event rates 
p.cpiu.be.ppl <- risk.adjust.be(rf = mymodel.1.full, status = train_b$i.sev_died.1, rt = train_b$rt_1p, alpha.tm = 0)

train_b$p.hat <- 1-exp(-p.cpiu.be.ppl) # predicted event probabilities 
train_b$ni.sca <- as.numeric(train_b$i.sev_died.1)-1
...
new.p <- risk.adjust.new.be(rf=mymodel.1.full, status = train_b$i.sev_died.1, rt = train_b$rt_1p, pre.mem = new.pred, k = 2, alpha.tm = 0)
