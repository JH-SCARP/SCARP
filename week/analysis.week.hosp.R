...
i = 2
  ...
  rf.df.1$i.sev_died.7 <- trainData$i.sev_died.7
  rf.df.1 <- rf.df.1[,!names(rf.df.1) %in% c("sev_died_within_7_days")]
  
  set.seed(321)
  node.size <- round((dim(trainData)[1])*0.1)
  mymodel.1 <- rfSLAM::rfsrc(i.sev_died.7 ~., data = rf.df.1, nodesize = node.size, ntree = 100, na.action = "na.impute", splitrule="poisson.split1", risk.time = trainData$rt_7p, stratifier=rf.df.1$q6, membership = TRUE, bootstrap = "by.user", samp = rfslam.samp.1)
  
  ## obtain predicted event rates 
  
  p.cpiu.be.ppl <- risk.adjust.be(rf = mymodel.1, status = rf.df.1$i.sev_died.7, rt = trainData$rt_7p, alpha.tm = 0)
  
  ### Calibrate predictions
  
  rf.df.1$p.hat <- 1-exp(-p.cpiu.be.ppl) # predicted event probabilities 
  rf.df.1$ni.sca <- as.numeric(rf.df.1$i.sev_died.7)-1
  ...
  test <- test[,!names(test) %in% c("sev_died_within_7_days")]
  
  new.pred <- predict(mymodel.1, newdata = test, na.action = "na.impute", membership = TRUE)
  
  new.p <- risk.adjust.new.be(rf=mymodel.1, status = rf.df.1$i.sev_died.7, rt = trainData$rt_7p, pre.mem = new.pred, k = 2, alpha.tm = 0)
...
g1 <- mutate(db2, bin = ntile(p.hat, 10)) %>% 
  ...
  geom_abline() + geom_smooth(method = "loess", se = FALSE, linetype = "dashed", 
                              color = "black") +
  #geom_smooth(aes(x = p.hat, y = as.numeric(i.sev_died.7) - 1), color = "red", se = FALSE, method = "loess") + 
  xlab("") +
...
# week 1
t0 <- db2 %>% filter(q6 <= 27)
auc.df[3,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))


# week 2
t0 <- db2 %>% filter(q6 > 27)
auc.df[4,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))
...
g2 <- ggplot(db2, aes(x = p.hat)) +
  geom_histogram(fill = "black", bins = 200) +
  scale_x_continuous(limits = c(0, 0.9), breaks = seq(0, 1, by = 0.1)) +
  xlab("Predicted Probability") +
  ylab("") +
  theme_minimal() +
...
# week 1
t0 <- db2 %>% filter(q6 <= 27)
auc.df[5,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))


# week 2
t0 <- db2 %>% filter(q6 > 27)
auc.df[6,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))
...
# week 1
t0 <- db2 %>% filter(q6 <= 27)
auc.df[7,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))


# week 2
t0 <- db2 %>% filter(q6 > 27)
auc.df[8,2:4] <- unname(ci.auc(t0$i.sev_died.7, t0$p.hat))
