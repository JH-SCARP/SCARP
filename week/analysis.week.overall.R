...
set.seed(321) 
start_time <- Sys.time()
mymodel.1.full <- rfSLAM::rfsrc(i.sev_died.7 ~., data = rf.df.1, nodesize = node.size, ntree = 100, na.action = "na.impute", splitrule="poisson.split1", risk.time = rf.cpiu.1$rt_7p, stratifier=rf.df.1$q6, membership = TRUE, bootstrap = "by.user", samp = rfslam.samp.1, var.used = "by.tree")
end_time <- Sys.time()

## obtain predicted event rates 
p.cpiu.be.ppl <- risk.adjust.be(rf = mymodel.1.full, status = rf.df.1$i.sev_died.7, rt = rf.cpiu.1$rt_7p, alpha.tm = 0)
...
rf.cpiu.1$ni.sca <- as.numeric(rf.cpiu.1$i.sev_died.7)-1
...
# week 1
t0 <- db2 %>% filter(q6 <= 27)
auc.df[1,2:4] <- unname(ci.auc(t0$sev_died_within_7_days, t0$risk))


# week 2
t0 <- db2 %>% filter(q6 > 27)
auc.df[2,2:4] <- unname(ci.auc(t0$sev_died_within_7_days, t0$risk))
...
g1 <- mutate(db2, bin = ntile(risk, 10)) %>% 
  ...
  mutate(n = n(), # Get ests and CIs
         bin_pred = mean(risk), 
         bin_prob = mean(sev_died_within_7_days), 
  ...
  geom_smooth(aes(x = risk, y = sev_died_within_7_days), 
              color = "gray", se = FALSE, method = "loess") + 
  # loess fit through estimates
  xlab("") +
  ylab("Observed Probability") + theme_classic() + ggtitle("1-Week Risk Predictions") + theme(plot.title = element_text(hjust = 0.5), text = element_text(size=12))
